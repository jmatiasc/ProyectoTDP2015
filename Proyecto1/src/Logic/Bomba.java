...
public class Bomba {
	 ...
	 * 
	 * @param p
	 *            Posición inicial de la bomba.
	 * @param alcan
	 *            Alcance de la bomba.
	 * @param tab
	 *            Tablero al que pertenece la bomba.
	 * @param b
	 *            Bomberman al que estara ligado la bomba.
	 */
	...
	/**
	 * Calcula las posiciones alcanzadas, luego busca todas las paredes que
	 * esten dentro del alcance(x+alance,x-alcance,y+alcance,y-alcance), las
	 * agrega a una estructura y luego invoca a destruir Modificar la bomba del
	 * bomberman, agregarle un NULL
	 */
	public Celda[] activar() {
		...
		for (int i = 1; i < alcance + 1; i++) {
			...
			if (posArriba > 0)
				arriba[i - 1] = new Posicion(ubicacion.getEjeX(),
						ubicacion.getEjeY() - i);

			posAbajo = ubicacion.getEjeY() + 1;
			if (posAbajo < 12)
				abajo[i - 1] = new Posicion(ubicacion.getEjeX(),
						(ubicacion.getEjeY() + i));

			posIzquierda = ubicacion.getEjeX() - 1;
			if (posIzquierda > 0)
				izquierda[i - 1] = new Posicion((ubicacion.getEjeX() - i),
						ubicacion.getEjeY());

			posDerecha = ubicacion.getEjeY() + 1;
			if (posDerecha < 30)
				derecha[i - 1] = new Posicion((ubicacion.getEjeX() + i),
						ubicacion.getEjeY());
	...
	/**
	 * Calcula los puntos que otorga una Celda si es alcanzada por la explosión
	 * de la bomba.
	 * 
	 * @param arreglo
	 *            Arreglo de celdas.
